...
// Map icon names to GIF paths
const iconMap: Record<string, string> = {
  'pos': '/images/icons/dataphone.gif',
  'ecommerce': '/images/icons/online-shopping.gif',
  'inventory': '/images/icons/clipboard.gif',
  'restaurant': '/images/icons/restaurant.gif',
  'school': '/images/icons/school.gif',
  'crm': '/images/icons/crm.gif',
};

// Fallback icon if no match
const defaultIcon = '/images/icons/default.gif';
...
export function ProductGrid({ products }: ProductGridProps) {
  ...
  const getAltText = (iconName?: string) => {
    if (!iconName) return 'Product icon';
    const altMap: Record<string, string> = {
      'pos': 'POS System icon',
      'ecommerce': 'E-commerce icon',
      'inventory': 'Inventory Management icon',
      'restaurant': 'Restaurant Management icon',
      'school': 'School Management icon',
      'crm': 'CRM icon',
    };
    return altMap[iconName] || 'Product icon';
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {products.map((product, index) => (
        ...
        >
          <div className="p-6">
            <div className={`w-16 h-16 rounded-xl bg-gradient-to-r ${getColor(product.icon)} flex items-center justify-center mb-4 overflow-hidden`}>
              <Image 
                src={getIcon(product.icon)}
                alt={getAltText(product.icon)}
                width={40}
                height={40}
